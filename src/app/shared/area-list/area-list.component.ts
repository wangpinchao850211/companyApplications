...
import { combineLatest, startWith, mergeMap } from 'rxjs/operators';
...
export class AreaListComponent implements ControlValueAccessor, OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const val$ = province$.pipe(
      combineLatest([city$, district$, street$], (_p, _c, _d, _s) => {
        return {
          province: _p,
          city: _c,
          district: _d,
    ...
    this.districts$ = province$.pipe(
      combineLatest(city$, (p, c) => ({province: p, city: c})),
